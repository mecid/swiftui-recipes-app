...
struct RecipesService {
    let fetch: (String, Health, Int) -> AnyPublisher<[Recipe], RecipesServiceError>
}

extension RecipesService {
    static let live = RecipesService(fetch: fetch(query:health:page:))
    static let mock = RecipesService { _, _, _ in
        Just([Recipe.mock])
...
private func fetch(query: String, health: Health = .gluten, page: Int = 1) -> AnyPublisher<[Recipe], RecipesServiceError> {
